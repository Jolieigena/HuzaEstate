...
import { FeedbackSeverity, ProfessionalReviewRequest, REVIEW_TYPE_LABELS, RENOVATION_AREA_LABELS } from "@/lib/renovate/types";
import { formatDate, formatDateTime } from "@/lib/renovate/format";
import { useToast } from "@/lib/toast-context";
import { ReviewStatusBadge } from "@/components/renovate/StatusBadge";
import RequestReviewModal from "@/components/renovate/RequestReviewModal";
...
function FeedbackItem({ projectId, review, feedbackId }: { projectId: string; review: ProfessionalReviewRequest; feedbackId: string }) {
  ...
  const [replyText, setReplyText] = useState(feedback.response ?? "");
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-4">
      <div className="flex items-start justify-between gap-3 mb-2">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {feedback.authorProfession} · {formatDateTime(feedback.createdAt)}
          </p>
        </div>
      ...
      <p className="text-sm text-slate-600 bg-slate-50 rounded-lg p-2.5 mb-2">Recommendation: {feedback.recommendation}</p>
      {feedback.areaKey && <p className="text-xs text-slate-400 mb-2">Related to: {RENOVATION_AREA_LABELS[feedback.areaKey]}</p>}
      {feedback.inspectionRequired && <p className="text-xs font-bold text-amber-700 bg-amber-50 rounded-lg px-2.5 py-1 inline-block mb-2">Inspection required</p>}
      {feedback.response && !replyOpen && <div className="bg-slate-50 rounded-lg p-3 text-sm text-slate-600 mb-2">Your reply: {feedback.response}</div>}
        ...
        <div className="mb-2">
          ...
          <div className="flex gap-2 mt-2">
            <button
              type="button"
              onClick={() => {
                RenovationProjectService.replyToFeedback(projectId, review.id, feedback.id, replyText);
                setReplyOpen(false);
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          type="button"
          onClick={() => RenovationProjectService.markFeedbackAddressed(projectId, review.id, feedback.id, !feedback.addressed)}
          className="text-xs font-bold text-slate-500 hover:text-slate-800"
        >
          {feedback.addressed ? "Marked as addressed" : "Mark as addressed"}
        </button>
        <Link href={`/studio/renovate/${projectId}/designer`} className="text-xs font-bold text-slate-500 hover:text-slate-800">
          Create revision
        </Link>
      </div>
    </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm max-w-xl">
            Huza AI produces conceptual renovation directions, not approved technical designs. A qualified professional should review structural, electrical, plumbing, safety and permit-related work before execution.
          </p>
        </div>
            ...
            <div key={review.id} className="bg-white border border-slate-100 rounded-2xl shadow-sm p-6">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div>
                  ...
                  {review.areasRequiringReview.length > 0 && (
                    <p className="text-xs text-slate-400 mt-1">Areas: {review.areasRequiringReview.map((a) => RENOVATION_AREA_LABELS[a]).join(", ")}</p>
                  )}
                </div>
              ...
              {review.questions && <p className="text-sm text-slate-600 bg-slate-50 rounded-xl p-3 mb-4">{review.questions}</p>}
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          RenovationProjectService.cancelReview(project.id, cancelTarget.id);
