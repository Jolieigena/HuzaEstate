...
import { DocumentCategory, DOCUMENT_CATEGORY_LABELS, ProjectDocument } from "@/lib/renovate/types";
import { formatBytes, formatDate } from "@/lib/renovate/format";
import { useToast } from "@/lib/toast-context";
import UploadDocumentModal from "@/components/renovate/UploadDocumentModal";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-black text-slate-900 mb-1">Renovation documents</h1>
          <p className="text-slate-500 text-sm">Photographs, floor plans, concepts, scope, budget and review outputs, all in one place.</p>
        </div>
        <div className="flex gap-2">
          <Link
            href={`/studio/renovate/${project.id}/documents/summary`}
            className="bg-white border border-slate-200 hover:border-slate-300 text-slate-700 font-bold px-5 py-3 rounded-xl transition-colors whitespace-nowrap"
          >
            Renovation Summary
          </Link>
            ...
            <tbody>
              {filtered.map((doc) => (
                <tr key={doc.id} className="border-b border-slate-50 last:border-b-0">
                  ...
                  <td className="p-4 text-slate-500">{DOCUMENT_CATEGORY_LABELS[doc.category]}</td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-3 text-xs font-bold">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          RenovationProjectService.logDocumentDownload(project.id, doc.name);
      ...
      <PromptModal
        ...
        onSubmit={(value) => {
          ...
          RenovationProjectService.renameDocument(project.id, renameTarget.id, value);
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {(Object.entries(DOCUMENT_CATEGORY_LABELS) as [DocumentCategory, string][]).map(([key, label]) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    RenovationProjectService.moveDocumentCategory(project.id, moveTarget.id, key);
                    setMoveTarget(null);
      ...
      <ConfirmModal
        ...
        description="This permanently removes this document. This cannot be undone."
        onConfirm={() => {
          if (!deleteTarget) return;
          RenovationProjectService.deleteDocument(project.id, deleteTarget.id);
