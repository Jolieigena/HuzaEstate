...
import { calculateBudget } from "@/lib/build/budget";
import { FINISH_LEVEL_LABELS, FinishLevel } from "@/lib/build/types";
import { formatCompactRwf, formatCurrency, formatDateTime } from "@/lib/build/format";
...
export default function BudgetPage() {
  ...
  const defaultArea = selectedConcept?.metrics.floorAreaSqm ?? project.brief.plot.areaSqm ?? 150;

  const [area, setArea] = useState(project.budget?.totalAreaSqm ?? defaultArea);
  const [finishLevel, setFinishLevel] = useState<FinishLevel>(project.budget?.finishLevel ?? project.brief.budget.finishLevel);
  const [contingency, setContingency] = useState(project.budget?.contingencyPct ?? 10);
  const [landscaping, setLandscaping] = useState(project.budget?.includeLandscaping ?? project.brief.budget.landscapingIncluded);
  const [furniture, setFurniture] = useState(project.budget?.includeFurniture ?? project.brief.budget.furnitureIncluded);
  const [professionalFees, setProfessionalFees] = useState(project.budget?.includeProfessionalFees ?? project.brief.budget.professionalFeesIncluded);
  ...
  const liveEstimate = useMemo(
    ...
    [area, finishLevel, contingency, landscaping, furniture, professionalFees]
  );

  useEffect(() => {
    if (!project.budget) {
      BuildProjectService.recalculateBudget(project.id, {
        totalAreaSqm: area,
        finishLevel,
        contingencyPct: contingency,
        includeLandscaping: landscaping,
  ...
  const hasChanges =
    saved &&
    (saved.totalAreaSqm !== area || saved.finishLevel !== finishLevel || saved.contingencyPct !== contingency || saved.includeLandscaping !== landscaping || saved.includeFurniture !== furniture || saved.includeProfessionalFees !== professionalFees);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-black text-slate-900 mb-1">Indicative project budget</h1>
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-6 grid sm:grid-cols-4 gap-4 text-sm">
        <div>
          <p className="text-slate-400 font-semibold">Cost per sqm</p>
          <p className="font-bold text-slate-900">{formatCurrency(liveEstimate.costPerSqm)}</p>
        </div>
        <div>
          <p className="text-slate-400 font-semibold">Floor area</p>
          <p className="font-bold text-slate-900">{liveEstimate.totalAreaSqm} sqm</p>
        </div>
        <div>
          <p className="text-slate-400 font-semibold">Location assumption</p>
          <p className="font-bold text-slate-900">{liveEstimate.locationAssumption}</p>
        </div>
        <div>
          <p className="text-slate-400 font-semibold">Last calculated</p>
      ...
      <div className="flex flex-wrap gap-2">
        <span className="text-xs font-bold bg-[#2ec440]/10 text-[#2ec440] px-3 py-1.5 rounded-full">AI indicative estimate — available</span>
        <span className="text-xs font-bold bg-slate-100 text-slate-400 px-3 py-1.5 rounded-full">Professional estimate — not yet requested</span>
        <span className="text-xs font-bold bg-slate-100 text-slate-400 px-3 py-1.5 rounded-full">Contractor quotation — not yet requested</span>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4">
        <p className="text-sm text-slate-600 leading-relaxed">
          ...
          This estimate supports early planning only. Material prices, labour, site conditions, professional fees and final specifications can change the construction cost.
        </p>
      </div>
    </div>
  );
