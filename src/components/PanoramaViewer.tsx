...
export default function PanoramaViewer({ panoUrl, className = '' }: PanoramaViewerProps) {
  ...
  return (
    <div className={className}>
      {/* Own positioning context, separate from `className` above — see
       *  the identical note in SplatViewer.tsx: that prop is always the
       *  caller's own positioning ("absolute inset-0"), so putting
       *  `relative` on the same element produced conflicting classes. */}
