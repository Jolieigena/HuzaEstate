...
export default function ProfessionalShell({ children }: { children: React.ReactNode }) {
  ...
  // A demo/fixture account (see DEMO_ACCOUNTS) has a matching row in the local mock workspace
  // and goes through the mock apply/approve flow below, unchanged. A real professional account
  // (created via POST /auth/admin/users) has no such row — its "approval" already happened when
  // an administrator created it, so it skips the mock apply flow entirely and instead is gated
  // on completing its real public profile (see PUT /professionals/me, account.profileCompleted).
  const isRealProfessional = account?.roles.includes("professional") ?? false;

  useEffect(() => {
    if (!isAuthReady || !account || applicationRoute || isRealProfessional) return;
  ...
  }, [account, applicationRoute, isAuthReady, isRealProfessional, pathname, profile, router]);

  return (
    <RequireAuth>
      {applicationRoute ? children : isRealProfessional ? (
